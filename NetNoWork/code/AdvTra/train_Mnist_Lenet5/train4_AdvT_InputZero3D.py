...
modelpath = './train4_AdvT_InputZero3D.pkl'
...
for epoch in range(param['num_epochs']):
    ...
    for t, (x, y) in enumerate(loader_train):
        ...
        loss = (gradXloss1 + gradXloss2)/2 + (criterion(out, y_var) + criterion(net(X_adv.cuda()), y_var)) / 2
        optimizer.zero_grad()  # 把上一轮的梯度清零
        loss.backward()  # 反向传播求导
        optimizer.step()  # 优化参数

        # if (t + 1) % 100 == 0:  # 每训练10批数据看一下loss
        #     print('t = %d, loss = %.8f, gradXloss1=%.8f, gradXloss2=%.8f' % (t + 1, loss.item(), gradXloss1, gradXloss2))
        if (t + 1) % 1 == 0:  # 每训练m批数据看一下测试精度，保存模型
            num_correct, num_samples, acc = Optimizer.test(net, loader_test)
            print('[train4_AdvT_InputZ2D] E-t = %d-%d, loss = %.8f, gradXloss1=%.8f, gradXloss2=%.8f ,'
                  'pre test_acc=%.4f%%' % (epoch + 1,
            loss.item(), gradXloss1, gradXloss2, num_samples, acc))
